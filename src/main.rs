...
fn main() -> anyhow::Result<()> {
    ...
    let source: Source;
    let watch_path: Option<PathBuf>;
    if let Some(_flake) = args.flake {
        todo!("flake mode");
    } else if let Some(_expr) = args.expr {
        todo!("expr mode");
        ...
        if path.as_os_str() == "-" {
            ...
            watch_path = if args.watch { Some(path.clone()) } else { None };
    ...
    let _watcher: Option<RecommendedWatcher> = match watch_path.as_ref() {
        Some(p) => Some(input::watcher::start(p, tx.clone())?),
        None => None,
    };
